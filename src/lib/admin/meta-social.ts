...
async function publishFacebookReel(
  ...
  // Attendre publishing_phase complète (évite un ID “ok” encore invisible / en traitement).
  if (finishBody.video_state === 'PUBLISHED') {
    let publishedOk = false;
    for (let attempt = 0; attempt < 45; attempt += 1) {
      await new Promise((r) => setTimeout(r, 2000));
      const statusRes = await graphJson(
        `${GRAPH}/${videoId}?fields=status,published,privacy&access_token=${encodeURIComponent(token)}`,
      );
      const status = statusRes.status as
        | {
            video_status?: string;
            publishing_phase?: { status?: string; publish_status?: string; error?: { message?: string } };
            processing_phase?: { status?: string; error?: { message?: string } };
          }
        | undefined;
      if (status?.publishing_phase?.error?.message) {
        throw new Error(`Facebook Reels publication : ${status.publishing_phase.error.message}`);
      }
      if (status?.processing_phase?.error?.message) {
        throw new Error(`Facebook Reels traitement : ${status.processing_phase.error.message}`);
      }
      const phase = status?.publishing_phase?.publish_status || status?.publishing_phase?.status;
      if (statusRes.published === true || phase === 'published' || phase === 'complete') {
        publishedOk = true;
        break;
      }
      if (status?.video_status === 'error') {
        throw new Error('Facebook Reels : statut vidéo = error après finish.');
      }
    }
    if (!publishedOk) {
      throw new Error(
...
/** Diagnostic Graph : Page / app / derniers Reels (sans logger le token). */
export async function diagnoseMetaFacebookVisibility(connection: MetaSocialConnection) {
  if (!connection.accessToken || !connection.pageId) {
    throw new Error('Meta non connectée.');
  }
  const token = connection.accessToken;
  const pageId = connection.pageId;
  ...
  let appName: string | null = typeof debugData.application === 'string' ? debugData.application : null;
  const appId = debugData.app_id ? String(debugData.app_id) : null;
  if (appId && process.env.META_APP_ID && process.env.META_APP_SECRET) {
    try {
      const appToken = `${process.env.META_APP_ID.trim()}|${process.env.META_APP_SECRET.trim()}`;
      const appInfo = await graphJson(
        `${GRAPH}/${encodeURIComponent(appId)}?fields=id,name&access_token=${encodeURIComponent(appToken)}`,
      );
  ...
  const page = await graphJson(
    `${GRAPH}/${encodeURIComponent(pageId)}?fields=id,name,is_published,fan_count,link&access_token=${encodeURIComponent(token)}`,
  );
  const posts = await graphJson(
    `${GRAPH}/${encodeURIComponent(pageId)}/published_posts?fields=id,message,created_time,permalink_url,is_published,application&limit=5&access_token=${encodeURIComponent(token)}`,
  );
  const recent = Array.isArray(posts.data)
    ? (posts.data as Array<Record<string, unknown>>).slice(0, 5).map((p) => {
        const application = p.application as { name?: string; id?: string } | undefined;
        return {
          id: String(p.id || ''),
          ...
          preview: typeof p.message === 'string' ? p.message.slice(0, 80) : '',
        };
      })
    : [];
  ...
  return {
    ...
    pageName: typeof page.name === 'string' ? page.name : null,
    pagePublished: page.is_published !== false,
    fanCount: typeof page.fan_count === 'number' ? page.fan_count : null,
    pageLink: typeof page.link === 'string' ? page.link : null,
    tokenValid: debugData.is_valid !== false,
    tokenType: debugData.type || null,
    appId,
    appName,
    scopes: Array.isArray(debugData.scopes) ? debugData.scopes : [],
    recentPosts: recent,
    warning: META_APP_LIVE_WARNING,
  };
}
