...
export async function generateSocialPhotoForPost(
  ...
  opts: {
    ...
    /** Photos à éviter (régénération Feed / cover). */
    excludeLibraryPaths?: Iterable<string>;
  },
): Promise<
...
> {
  ...
  if (opts.preferLibrary !== false && post.format === 'feed' && !opts.forceNanoBanana) {
    const libPath = pickLibraryPath({
      ...
      excludePaths: opts.excludeLibraryPaths,
      seed: variationSeed,
  ...
  // Cover carousel (preferLibrary) — même logique biblio avec seed / anti-répétition.
  if (opts.preferLibrary && !opts.forceNanoBanana) {
    const libPath = pickLibraryPath({
      usedPaths: opts.usedLibraryPaths,
      excludePaths: opts.excludeLibraryPaths,
      seed: variationSeed,
