...
export function weekPlanSummary(networks: SocialNetwork[]): string {
  ...
  if (networks.includes('instagram') || networks.includes('tiktok')) {
    parts.push('TT = miroir Reels IG');
  }
...
export function resolveGenerationNetworks(filter: SocialNetwork): SocialNetwork[] {
  // Facebook / TikTok = miroir Instagram : on génère les posts IG.
  if (filter === 'facebook' || filter === 'tiktok') return ['instagram'];
  return [filter];
}
