...
export async function buildAcquisitionOverview(
  ...
  const [ga4, gsc, stripe, supa, pixel, schemaReady] = await Promise.all([
  ...
  const trials = stripe.ok ? stripe.data.activeTrials : supa.ok ? supa.data.trialingCount : null;
  const paid = stripe.ok ? stripe.data.activePaid : supa.ok ? supa.data.paidCount : null;
  ...
  const mrr = stripe.ok ? stripe.data.mrrEur : null;
  const referral = stripe.ok ? stripe.data.referralConversions30d : null;

  // CAC simplifié : MRR collecté / nouveaux payants (proxy — affiché avec hint)
  const cac =
    paid && paid > 0 && mrr != null ? Math.round((mrr / paid) * 100) / 100 : null;
  const ltv = mrr != null && paid && paid > 0 ? Math.round((mrr / paid) * 12 * 100) / 100 : null;
  ...
  const kpis: AcquisitionKpi[] = [
    {
      ...
      label: 'CAC (proxy)',
      value: formatEur(cac),
      hint: 'MRR du mois ÷ payants actifs — proxy jusqu’à budget ads branché.',
      tone: cac != null && cac > 80 ? 'watch' : 'neutral',
    },
    {
      id: 'ltv',
      label: 'LTV (proxy 12 mois)',
      value: formatEur(ltv),
      hint: 'Basé sur MRR actuel × 12 — à affiner avec churn réel.',
    },
    {
      id: 'trial_paid',
      label: 'Essai → Payant',
