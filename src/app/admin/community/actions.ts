...
import { addLibraryPathAliases, hashVariationSeed, isBoardLibraryPath } from '@/lib/admin/library-path-utils';
...
export async function updateSocialPostImageAction(postId: string, imagePath: string | null) {
  ...
  const isAllowed =
    ...
    isBoardLibraryPath(imagePath) ||
    imagePath.startsWith('http') ||
    imagePath.startsWith('/library/social/');
  const safe = isAllowed ? imagePath : pickLibraryImage(hashVariationSeed(postId, Date.now()));
  await saveSocialCommsBoard({
    ...board,
    posts: board.posts.map((post) =>
      post.id === postId
...
export async function generateSocialImageAction(postId: string, feedbackOverride?: string, slideIndex = 0) {
  ...
  if (isLibrarySlot) {
    const { generateSocialPhotoForPost } = await import('@/lib/admin/social-ai-image');
    const currentPath = isCarousel ? post.carouselPaths?.[idx] || post.imagePath : post.imagePath;
    const result = await generateSocialPhotoForPost(
      {
        ...post,
        format: post.format === 'carousel' ? 'carousel' : 'feed',
        imageHint: slideHint,
        title: slideHint,
      ...
      {
        feedback,
        variationSeed: hashVariationSeed(post.id, idx, 'regen-library', Date.now()),
        usedLibraryPaths: usedLibrary,
        preferLibrary: true,
        forceNanoBanana: false,
        allowUnsplash: false,
        libraryFolder: isCarousel ? 'portraits' : undefined,
        libraryThemeHint: isCarousel
          ? 'portrait Alejandra confiance identité'
          : post.imageHint || post.title || 'portraits',
        excludeLibraryPaths: currentPath ? [currentPath] : undefined,
      },
    );
    ...
    return {
      ok: true as const,
      message: `Nouvelle photo bibliothèque${slideNote}.`,
    };
  }
  ...
  const result = await generateSocialAiImage(
    ...
    feedback,
    hashVariationSeed(post.id, idx, 'regen-ai', Date.now()),
  );
  if (!result.ok) return result;
  ...
  try {
    ...
    if (slot.mediaKind !== 'video_brief') {
      ...
      if (slot.mediaKind === 'carousel') {
        ...
        const r = await generateSocialPhotoForPost(target, {
          variationSeed: hashVariationSeed(
            target.id,
            'feed',
