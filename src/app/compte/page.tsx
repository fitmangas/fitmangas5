...
export default async function ComptePage({
  ...
  return (
    <div className="min-h-screen bg-brand-beige px-6 py-16">
      <div className="mx-auto max-w-lg rounded-[32px] border border-brand-ink/[0.06] bg-white p-10 shadow-[0_10px_40px_rgba(0,0,0,0.04)]">
        ...
        {checkoutOk && (
          <p className="mb-6 rounded-2xl border border-brand-accent/20 bg-brand-sand/20 px-4 py-3 text-sm text-brand-ink/80">
            Paiement enregistré. Tu recevras la confirmation par e-mail (reçu Stripe). L’accès replays / calendrier sera
            branché dans les prochaines étapes produit.
          </p>
        )}
