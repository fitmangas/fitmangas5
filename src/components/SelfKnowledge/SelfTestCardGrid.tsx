...
/** Grille côte à côte — cartes verticales élancées aérées (style V1). */
export function SelfTestCardGrid({ locale, cards, compact = false, vertical = false }: Props) {
  const imgClass = vertical
    ? 'aspect-[3/4] w-full min-h-[240px] sm:min-h-[300px]'
    : compact
  ...
  return (
    <section
      className={`mx-auto max-w-5xl px-5 ${compact ? 'pb-4 pt-2' : 'pb-8 pt-6 sm:pb-12 sm:pt-10'}`}
    ...
    >
      ...
      <ul
        className={`mt-6 grid gap-5 sm:mt-8 sm:grid-cols-2 sm:gap-7 ${
          ...
          <li key={card.slug}>
            ...
            >
              ...
              <div className="flex flex-1 flex-col p-5 sm:p-6">
                <h3 className="font-serif text-[1.25rem] italic leading-snug text-brand-ink sm:text-[1.4rem]">
                  {card.title}
                </h3>
                <p className="mt-2.5 flex-1 text-[13px] leading-relaxed text-brand-ink/60">
                  {card.description}
                </p>
                {card.sourceLine ? (
                  <p className="mt-2 text-[10px] tracking-wide text-brand-ink/35">{card.sourceLine}</p>
                ) : null}
                <Link
                  href={card.href}
                  data-testid={`grid-cta-${card.slug}`}
                  className="mt-5 inline-flex w-full items-center justify-center rounded-full bg-[#c45d3e] px-5 py-3 text-[11px] font-bold uppercase tracking-[0.16em] text-white shadow-[0_10px_24px_rgba(196,93,62,0.28)] transition hover:bg-[#b35338]"
                >
                  {card.cta} →
                </Link>
              </div>
