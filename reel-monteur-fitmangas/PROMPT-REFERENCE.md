...
> **Source de vérité code** : `src/lib/admin/reel-prompt-reference.ts` (bouton « Copier prompt Claude Code »).
...
```
Monte un nouveau Reel FitMangas au STANDARD VALIDÉ (réf. dolor-espalda + trop-tard + liberte).
...
punch-in + B-roll exercices en vignette arrondie muette calée sur la phrase (clip exact si nommé, sinon
catégorie) + SFX variés par type + musique lounge subliminale qui respire), Follow @fit.mangas, CTA pile
3 cartes en zone sombre, audio −16 dB stable sans RNNoise/anlmdn, zones mortes IG, export H.264 SDR Rec.709 local.

Validé sur : dolor-espalda (premier Reel, ES), trop-tard (témoignage FR, format split screen LMDM
complet) et liberte (réf. B-roll exercices).
